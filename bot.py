...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.photo:
        log_message(user, "photo", msg.caption or "(fotoğraf)",
                    extra={"file_id": msg.photo[-1].file_id})
    elif msg.video:
        log_message(user, "video", msg.caption or msg.video.file_name or "(video)",
                    extra={"file_id": msg.video.file_id})
    elif msg.document:
        log_message(user, "document", msg.document.file_name or "(dosya)",
                    extra={"file_id": msg.document.file_id})
    elif msg.audio:
        log_message(user, "audio", msg.audio.file_name or "(ses)",
                    extra={"file_id": msg.audio.file_id})
    elif msg.voice:
        log_message(user, "voice", "(sesli mesaj)",
                    extra={"file_id": msg.voice.file_id})
    elif msg.animation:
        log_message(user, "animation", "(GIF)",
                    extra={"file_id": msg.animation.file_id})
    elif msg.sticker:
