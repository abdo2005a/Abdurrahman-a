...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("user|") and is_main_admin(uid):
        ...
        if action == "export":
            ...
            def to_ascii(s):
                """Unicode metni ASCII-safe yap."""
                if not s: return "-"
                try:
                    # Önce normal string olarak dene
                    clean = str(s).encode("ascii", "ignore").decode("ascii").strip()
                    return clean if clean else f"[ID:{target}]"
                except:
                    return f"[ID:{target}]"

            name     = to_ascii(u.get("full_name") or u.get("first_name") or "")
            username = to_ascii(u.get("username") or "")
            cls_str  = to_ascii(class_label(target))
            grp_str  = to_ascii(get_user_group(target)) if "get_user_group" in dir() else "-"
            ...
            for m in msgs:
                ...
                lbl   = TYPE_MAP.get(t, t)
                zaman = m.get("time", "")
                cont  = to_ascii(m.get("content", ""))
                if not cont or cont == "-":
                    cont = f"[{lbl}]"
                lines.append(f"  {zaman:<22} {lbl:<18} {cont[:80]}")
