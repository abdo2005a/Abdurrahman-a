...
# Railway Volume /data'ya mount edilmeli. Yoksa yerel klasör kullan.
...
async def _send_refresh(update, content, path, uid, extra=""):
    ...
    text    = format_folder(current, path, uid, admin=True)
    if extra: text += f"\n\n{extra}"
    # Sadece onay mesajı gönder, ANA SAYFA tekrarı değil
    if extra:
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Reply keyboard butonları bu handler'a düşmemeli ama düşerse engelle
    if text in ALL_REPLY_BTNS:
        return
...
def main():
    ...
    if "/data" in BASE_DIR:
        print("✅ Railway Volume aktif - dosyalar kalıcı kaydediliyor")
    else:
        print("⚠️  Railway Volume YOK - dosyalar geçici (deploy'da silinir)")
        print("    Çözüm: Railway → Settings → Volumes → Mount path: /data")
