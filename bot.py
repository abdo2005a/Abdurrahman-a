...
TR = {
    ...
    # AI
    "btn_ai":            "🤖 AI Sohbet",
    "ai_mode_on":        "🤖 AI Sohbet modu açık. Mesajını yaz.\n/menu ile çıkabilirsin.",
    "ai_error":          "⚠️ Şu an yanıt veremiyorum, lütfen tekrar dene.",
    "ai_admin_req":      "📨 Talebiniz yöneticiye iletildi.",
    "ai_global_off":     "🤖 AI şu an kapalı.",
...
AR = {
    ...
    # AI
    "btn_ai":            "🤖 محادثة AI",
    "ai_mode_on":        "🤖 وضع المحادثة مفعّل. اكتب رسالتك.\nاكتب /menu للخروج.",
    "ai_error":          "⚠️ لا أستطيع الرد الآن، يرجى المحاولة مجدداً.",
    "ai_admin_req":      "📨 تم إرسال طلبك إلى المسؤول.",
    "ai_global_off":     "🤖 المساعد الذكي غير متاح حالياً.",
...
AI_SYSTEM_PROMPT = (
    "أنت مساعد ذكاء اصطناعي ودود ومفيد لبوت تيليغرام. "
    "تحدث دائماً بالعربية بأسلوب واضح ومناسب. "
    "ساعد المستخدمين في أسئلتهم واستفساراتهم. "
    "إذا طلب المستخدم شيئاً يحتاج تدخل المسؤول (شكوى، طلب خاص، مشكلة)، "
    "أضف [ADMIN_REQ] في نهاية ردك مع ملخص قصير للطلب. "
    "لا تضف [ADMIN_REQ] للأسئلة العادية."
)

async def get_gemini_response(user_id: str, message: str) -> str:
    import aiohttp
    if not GEMINI_API_KEY:
        return AR["ai_error"]
    history     = load_ai_history()
    uid_history = history.get(str(user_id), [])
    contents    = []
    for h in uid_history[-16:]:
...
async def notify_admin_ai_request(context, user, summary: str):
    users = load_users()
    ...
    kb = [[InlineKeyboardButton(TR["msg_btn"], callback_data=f"dm_quick|{user.id}")]]
    try:
        await context.bot.send_message(ADMIN_ID, text, reply_markup=InlineKeyboardMarkup(kb))
    except Exception as e:
        logger.error(f"Admin AI bildirimi hatası: {e}")
...
def reply_kb(uid):
    ...
    if is_main_admin(uid):
        ...
        # AI butonu — ayrı satır, ayardan kontrol edilir
        if ub.get("btn_ai", True):
            rows.append([KeyboardButton(AR["btn_ai"])])
        if not rows: rows = [[]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Yeni kullanıcı bildirimi — sadece normal kullanıcılar için
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── AI Sohbet (sadece kullanıcılar) ─────────────
    if text == AR["btn_ai"] and not is_admin(uid):
        s = load_settings()
        if not s.get("user_buttons", {}).get("btn_ai", True):
            await update.message.reply_text(AR["ai_global_off"]); return
        context.user_data["mode"] = "ai_chat"
        await update.message.reply_text(AR["ai_mode_on"])
        return
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("user|") and is_main_admin(uid):
        ...
        if action == "info":
            ...
            media_cnt = sum(1 for m in msgs if m["type"] != "msg")
            ...
            info = (
                f"👤 {name}{un}\n🆔 {target}\n"
                f"{TR['user_last']} {last}\n"
                f"{TR['user_msgs'].format(txt_cnt, media_cnt)}"
            )
    ...
    if cb.startswith("set|") and is_main_admin(uid):
        ...
        if action == "toggle_btn":
            key = cb.split("|")[2]
            s   = load_settings()
            ub  = s.setdefault("user_buttons", {"btn_search": True, "btn_help": True, "btn_ai": True})
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "user_msg_to_admin" and not is_admin(uid):
        ...
        name  = u.get("full_name") or u.get("first_name") or f"ID:{uid}"
        un    = f" @{u['username']}" if u.get("username") else ""
        notif = TR["admin_user_msg"].format(
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text: log_user_message(user,"msg",text)

    # ── AI sohbet modu ────────────────────────────────
    if text and context.user_data.get("mode") == "ai_chat":
        if not s.get("user_buttons", {}).get("btn_ai", True):
            await msg.reply_text(AR["ai_global_off"]); return
        await context.bot.send_chat_action(msg.chat_id, "typing")
        reply = await get_gemini_response(uid, text)
        if "[ADMIN_REQ]" in reply:
            parts   = reply.split("[ADMIN_REQ]", 1)
            clean   = parts[0].strip()
            summary = parts[1].strip() if len(parts) > 1 else text
            if clean: await msg.reply_text(clean)
            await msg.reply_text(AR["ai_admin_req"])
            await notify_admin_ai_request(context, user, summary or text)
        else:
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(text_f & ~reply_btn_f, handle_text),
        ],
        states={
