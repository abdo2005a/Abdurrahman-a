...
def folder_kb(path, folder, uid, page=0):
    ...
    # Kullanıcılar için — ana sayfada sadece kendi sınıflarının klasörü görünsün
    raw_folders = list(folder.get("folders", {}).items())
    if not path and not is_admin(uid):
        cls    = get_user_class(uid)
        cls_ar = CLASS_DEFS.get(cls, {}).get("ar", "") if cls else ""
        if cls_ar:
            # Sadece kullanıcının sınıfıyla eşleşen klasörü göster
            raw_folders = [
                (name, sub) for name, sub in raw_folders
                if cls_ar in name or name in cls_ar
            ]

    all_folders = raw_folders
    PAGE_SIZE   = 20
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cls:
        # Sınıf seçmemiş — sadece karşılama + sınıf seçimi göster, klasör YOK
        welcome = ""
        ...
        prompt = L(uid, "class_select_prompt")
        if welcome and not s.get("bot_photo_id"):
            prompt = welcome + "\n\n" + prompt
        await update.message.chat.send_message(prompt, reply_markup=InlineKeyboardMarkup(kb))
    else:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_admin(uid):
        ...
        if (subject or is_q) and len(text.strip()) >= 3:
            typing_msg = None
            try:
            ...
            if answer:
                ...
                if faqs:
                    ...
                    await update.message.reply_text(L(uid,"ai_no_answer"))
        else:
