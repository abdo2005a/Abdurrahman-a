...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("mgmt|") and is_main_admin(uid):
        ...
        if action == "admin_log_export":
            ...
            b.seek(0)
            await query.message.reply_document(b, filename=f"admin_log_{datetime.now().strftime('%Y%m%d')}.csv",
                caption="📋 Admin İşlem Kaydı")
            return ConversationHandler.END
        ...
        if action == "ulist":
            ...
            for uid_, u in u_d.items():
                if int(uid_) == ADMIN_ID: continue
                if filt.startswith("cls_"):
                    if get_user_class(uid_) != filt[4:]: continue
                elif filt.startswith("grp_"):
                    grp_main = filt[4:]
                    if not grps_d.get(uid_,"").startswith(grp_main): continue
                name  = u.get("full_name") or u.get("first_name") or f"ID:{uid_}"
                un    = f" @{u['username']}" if u.get("username") else ""
            ...
            lbl = {"all":"Tümü"}.get(filt, filt)
            await query.edit_message_text(
                f"👥 Kullanıcılar ({len(kb)-1}) — {lbl}:",
