...
def add_lab_week(week_start_str: str, group: str, note: str = ""):
    """Haftaya grup ata. week_start_str: YYYY-MM-DD (Pazartesi)."""
    schedule = load_lab_schedule()
    # Aynı hafta varsa güncelle
    for entry in schedule:
        if entry.get("week") == week_start_str:
            entry["group"] = group
            entry["note"]  = note
    ...
    schedule = sorted(schedule, key=lambda x: x["week"])
    save_lab_schedule(schedule)
...
def get_countdowns(cls: str = "", shift: str = "", uid: str = "") -> list:
    ...
    for c in cd:
        if c.get("ts", 0) > now:
            ...
            # Sınıf — ana veya ek sınıftan biri eşleşmeli (ya da herkese açık)
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔬 المختبر" and not is_admin(uid):
        week_entry = get_current_lab_week()
        schedule   = get_upcoming_lab_weeks(4)
        if not week_entry:
            msg_text = "🔬 المختبر\n\nلا يوجد جدول للمختبر هذا الأسبوع."
        else:
            grp = week_entry.get("group","?")
            note = week_entry.get("note","")
            msg_text = f"🔬 المختبر — هذا الأسبوع\n\n👥 المجموعة: {grp}\n"
            if note:
                msg_text += f"📝 {note}\n"
        if schedule:
            msg_text += "\n📅 الجدول القادم:\n"
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("admin_bcast|") and is_admin(uid) and not is_main_admin(uid):
        ...
        if ab_action == "send" or ab_action.startswith("cls|"):
            ...
            context.user_data["admin_bcast_grp"] = get_admin_grp(uid)
    ...
    if cb.startswith("lab|") and is_main_admin(uid):
        lab_parts = cb.split("|")
        lab_act   = lab_parts[1]
        if lab_act == "edit":
            week = lab_parts[2]
            kb = [
                [InlineKeyboardButton("B1", callback_data=f"lab|set|{week}|B1"),
                 InlineKeyboardButton("B2", callback_data=f"lab|set|{week}|B2"),
                 InlineKeyboardButton("B3", callback_data=f"lab|set|{week}|B3")],
                [InlineKeyboardButton("A1", callback_data=f"lab|set|{week}|A1"),
                 InlineKeyboardButton("A2", callback_data=f"lab|set|{week}|A2"),
                 InlineKeyboardButton("A3", callback_data=f"lab|set|{week}|A3")],
                [InlineKeyboardButton("C1", callback_data=f"lab|set|{week}|C1"),
                 InlineKeyboardButton("C2", callback_data=f"lab|set|{week}|C2"),
                 InlineKeyboardButton("C3", callback_data=f"lab|set|{week}|C3")],
                [InlineKeyboardButton("◀️ Geri", callback_data="mgmt|lab")],
            ]
            await query.edit_message_text(f"🔬 {week}\nHangi grup?", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
        if lab_act == "set":
            week  = lab_parts[2]
            group = lab_parts[3]
            add_lab_week(week, group)
            await query.answer(f"✅ {week} → {group}", show_alert=True)
            query.data = "mgmt|lab"
            return await callback(update, context)
    ...
    if cb == "mgmt|my_users" and is_admin(uid) and not is_main_admin(uid):
        ...
        for uid_, u in users_d.items():
            ...
            if adm_grp:
                ug = load_groups().get(uid_,"")
                if not ug.startswith(adm_grp): continue
        ...
        if not kb:
            await query.answer("لا يوجد طلاب في فئتك بعد", show_alert=True)
            return ConversationHandler.END
        await query.edit_message_text("👥 طلاب صفي:", reply_markup=InlineKeyboardMarkup(kb[:30]))
        return ConversationHandler.END
    ...
    if cb.startswith("mgmt|") and is_main_admin(uid):
        ...
        if action == "lab":
            schedule = get_upcoming_lab_weeks(8)
            from datetime import date, timedelta
            # Pazartesi listesi — sonraki 8 hafta
            today = date.today()
            mon   = today - timedelta(days=today.weekday())
            weeks = [(mon + timedelta(weeks=i)).strftime("%Y-%m-%d") for i in range(8)]
            sched_map = {e["week"]: e for e in load_lab_schedule()}
            kb = []
            for w in weeks:
                entry = sched_map.get(w)
                grp_lbl = f" → {entry['group']}" if entry else " → ?"
                kb.append([InlineKeyboardButton(
                    f"📅 {w}{grp_lbl}",
                    callback_data=f"lab|edit|{w}")])
            kb.append([InlineKeyboardButton("◀️ Geri", callback_data="nav|root")])
            await query.edit_message_text("🔬 Lab Programi (8 Hafta):", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
        ...
        if action == "admin_log_export":
            ...
            try:
                await context.bot.send_document(
                    int(uid),
                    document=b,
                    filename=f"admin_log_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
        ...
        if action == "users":
            # Filtre seçim ekranı: sınıf / grup
            kb = [
                [InlineKeyboardButton("📋 Tümü",        callback_data="mgmt|ulist|all")],
            ]
            for cls_id, cls_def in CLASS_DEFS.items():
                cnt = len(users_by_class(cls_id))
                kb.append([InlineKeyboardButton(
                    f"{cls_def['tr']}  ({cnt})",
                    callback_data=f"mgmt|ulist|cls_{cls_id}")])
            # Grup filtreleri
            grps_data = load_groups()
            for grp in ("A","B","C"):
                cnt = sum(1 for v in grps_data.values() if v.startswith(grp))
                if cnt: kb.append([InlineKeyboardButton(f"👥 Grup {grp}  ({cnt})", callback_data=f"mgmt|ulist|grp_{grp}")])
            kb.append([InlineKeyboardButton("🔍 İsimle / @username Ara", callback_data="mgmt|ulist|search")])
            kb.append([InlineKeyboardButton("◀️ Geri", callback_data="nav|root")])
            await query.edit_message_text("👥 Kullanıcılar — Filtre:", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
        ...
        if action == "ulist":
            ...
            # Sınıf seçildiyse alt filtre ekle: Sabahi/Masaiy + Grup
            if filt.startswith("cls_") and len(parts) == 3:
                cls_id = filt[4:]
                cls_name = get_class_display_name(cls_id)
                # Sabahi/Masaiy sayıları
                sab = sum(1 for u_ in u_d if get_user_class(u_)==cls_id and shfts.get(u_,"") in ("sabahi","sabah"))
                mas = sum(1 for u_ in u_d if get_user_class(u_)==cls_id and shfts.get(u_,"") in ("masaiy","gece"))
                tot = sum(1 for u_ in u_d if get_user_class(u_)==cls_id)
                kb = [
                    [InlineKeyboardButton(f"📋 الكل ({tot})",    callback_data=f"mgmt|ulist|{filt}|all")],
                    [InlineKeyboardButton(f"☀️ صباحي ({sab})",  callback_data=f"mgmt|ulist|{filt}|sabahi"),
                     InlineKeyboardButton(f"🌙 مسائي ({mas})",  callback_data=f"mgmt|ulist|{filt}|masaiy")],
                ]
                for grp in ("A","B","C"):
                    cnt = sum(1 for u_ in u_d if get_user_class(u_)==cls_id and grps_d.get(u_,"").startswith(grp))
                    if cnt: kb.append([InlineKeyboardButton(f"👥 Grup {grp} ({cnt})", callback_data=f"mgmt|ulist|{filt}|grp_{grp}")])
                kb.append([InlineKeyboardButton("◀️ Geri", callback_data="mgmt|users")])
                await query.edit_message_text(f"👥 {cls_name}:", reply_markup=InlineKeyboardMarkup(kb))
                return ConversationHandler.END

            # Kullanıcı listesi — filt + alt filt
            sub = parts[3] if len(parts) > 3 else "all"
            kb = []
            for uid_, u in u_d.items():
                if int(uid_) == ADMIN_ID: continue
                if filt.startswith("cls_") and get_user_class(uid_) != filt[4:]: continue
                if filt.startswith("grp_") and not grps_d.get(uid_,"").startswith(filt[4:]): continue
                if sub == "sabahi" and shfts.get(uid_,"") not in ("sabahi","sabah"): continue
                if sub == "masaiy" and shfts.get(uid_,"") not in ("masaiy","gece"): continue
                if sub.startswith("grp_") and not grps_d.get(uid_,"").startswith(sub[4:]): continue
                name  = u.get("full_name") or u.get("first_name") or f"ID:{uid_}"
                un    = f" @{u['username']}" if u.get("username") else ""
            ...
            if not kb:
                ...
                return ConversationHandler.END
            back_cb = f"mgmt|ulist|{filt}" if len(parts) > 3 else "mgmt|users"
            kb.append([InlineKeyboardButton("◀️ Geri", callback_data=back_cb)])
            await query.edit_message_text(
                f"👥 Kullanıcılar ({len(kb)-1}):",
                reply_markup=InlineKeyboardMarkup(kb[:40]))
            return ConversationHandler.END
    ...
    if cb.startswith("user|") and is_admin(uid):
        ...
        # Alt admin sadece extra_cls ve toggle_xcls yapabilir (can_view_users gerekli)
        if not is_main_admin(uid):
            allowed_sub = {"extra_cls", "toggle_xcls"}
            if action not in allowed_sub:
        ...
        if action == "export":
            ...
            if not HAS_OPENPYXL:
                ...
                w.writerow(["Rapor Tarihi",  datetime.now().strftime("%Y-%m-%d %H:%M")])
                w.writerow([])
                ...
                b2.seek(0)
                fn2 = f"rapor_{target}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
                try:
            ...
            if not HAS_OPENPYXL:
                ...
                bio2.seek(0)
                fname2 = f"rapor_{target}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
                try:
            ...
            wb.save(bio)
            bio.seek(0)
            fname = f"rapor_{target}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
            try:
        ...
        if action_n == "sel_subj":
            ...
            kb = [
                [InlineKeyboardButton("✍️ نص",    callback_data="notes|add|text"),
                 ...
                 InlineKeyboardButton("🎥 فيديو", callback_data="notes|add|video")],
    ...
    if cb.startswith("poll|") and is_main_admin(uid):
        ...
        if action == "create":
            if is_admin(uid) and not is_main_admin(uid):
                # Alt admin — sadece kendi sınıf/grubu
                adm_cls = get_admin_cls(uid)
                adm_grp = get_admin_grp(uid)
                # Hedefi otomatik belirle
                tgt_parts = []
                if adm_cls: tgt_parts.append(f"cls_{adm_cls}")
                else: tgt_parts.append("cls_all")
                tgt_parts.append("sft_all")
                if adm_grp: tgt_parts.append(f"grp_{adm_grp}")
                else: tgt_parts.append("grp_all")
                auto_target = "|".join(tgt_parts)
                context.user_data["poll_target"] = auto_target
                lbl = target_label(auto_target)
                kb = [
                ...
                # Süper admin — tam hedefleme
                kb = build_target_keyboard("poll|tgt", "cls")
                kb.append([InlineKeyboardButton(TR["cancel"], callback_data="close")])
                await query.edit_message_text("📊 اختر الفئة المستهدفة:", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "note_subject_input" and not is_admin(uid):
        ...
        lbl = subject if subject else "ملاحظة عامة"
        sent = await update.message.reply_text(
            f"📖 {lbl}\n─────\nاختر نوع الملاحظة:",
            reply_markup=InlineKeyboardMarkup(kb))
        context.user_data["last_inline_msg"] = sent.message_id
        return WAIT_FOLDER  # ConversationHandler açık kalsın, callback bekle

    # ── Kişisel Not — Metin notu kaydet ─────────────
    ...
    if action == "admin_bcast_msg" and is_admin(uid) and not is_main_admin(uid):
        ...
        tgt_grp = context.user_data.pop("admin_bcast_grp", None)
        context.user_data.pop("action", None)
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "add_note" and not is_admin(uid):
        ...
        fid = None
        cap = msg.caption or ""
        # Gönderilen medyayı algıla — kullanıcı yanlış tip seçmiş olabilir
        if msg.photo:
            ...
            cap = cap or msg.document.file_name or "dosya"
            note_type = "document"
        elif msg.video_note:
...
def main():
    ...
    async def _check_exam_reminders(ctx):
        """Sınavdan 1 gün önce kullanıcılara bildirim gönder."""
        from datetime import datetime as _dt, timedelta
        tomorrow = (_dt.now() + timedelta(days=1)).strftime("%d/%m/%Y")
        cds = load_countdowns()
        for cd in cds:
            if cd.get("date","") == tomorrow and not cd.get("notified"):
                name_cd = cd.get("name","Sinav")
                cls_cd  = cd.get("cls","")
                shft_cd = cd.get("shift","")
                targets = get_target_users(f"cls_{cls_cd}" if cls_cd else "all")
                if shft_cd:
                    shfts = load_shifts()
