...
DEFAULT_CONTENT = {
    "folders": {
        ...
        "الثاني":  {"folders": {}, "files": []},
        "الثالث": {"folders": {}, "files": []},
        "الرابع":  {"folders": {}, "files": []},
    },
    "files": [],
}
...
def _try_math_eval(expr: str) -> str | None:
    """
...
async def web_search_ddg(query: str) -> dict | None:
    """
...
async def web_search_wikipedia(query: str, lang: str = "ar") -> dict | None:
    """
...
def _subject_label(uid: str, subject: str | None) -> str:
    labels = {
...
def ai_match_faq(text: str) -> str | None:
...
def ai_match_rules(text: str) -> str | None:
...
def ai_respond(uid: str, text: str) -> str | None:
    """
...
def reply_kb(uid):
    ...
    if is_main_admin(uid):
        ...
        if ub.get("profile_btn", True):    row2.append(KeyboardButton(AR["profile_btn"]))
        if row2: rows.append(row2)
        # Favoriler + son görüntülenenler (sadece veri varsa göster)
        row3 = []
        if get_favorites(uid):             row3.append(KeyboardButton(AR["btn_favorites"]))
        if get_recently_viewed(uid):       row3.append(KeyboardButton(AR["btn_recent"]))
        if row3: rows.append(row3)
        # Liderboard
        row4 = [KeyboardButton(AR["btn_leaderboard"])]
        rows.append(row4)
        if not rows: rows = [[]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    old_main = context.user_data.get("main_msg_id")
    if old_main:
        try: await context.bot.delete_message(update.effective_chat.id, old_main)
        except: pass

    await update.message.chat.send_message("👋", reply_markup=reply_kb(uid))

    # ── Sınıf seçimi (sadece kullanıcılar, ilk kez) ──────
    if not is_admin(uid) and not get_user_class(uid):
        kb = [
    ...
    if not is_admin(uid):
        if s["maintenance"]:
            await update.message.chat.send_message(s.get("maintenance_text","🔧")); return
        if s.get("bot_photo_id"):
            await update.message.chat.send_photo(s["bot_photo_id"], caption=s.get("welcome_msg") or None)
    ...
    if text in (TR["ai_btn"], AR["ai_btn"]):
        if not is_admin(uid):
            ...
            if recs:
                ...
                txt += "💬 " + L(uid,"ai_faq_btn")
            kb = [[InlineKeyboardButton(L(uid,"ai_faq_btn"), callback_data="ai|faq")]]
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("class_pick|"):
        ...
        if cls in CLASS_DEFS:
            ...
            cls_name = CLASS_DEFS[cls]["ar"] if not is_main_admin(uid) else CLASS_DEFS[cls]["tr"]
            await query.edit_message_text(L(uid,"class_selected").format(cls_name))
        return ConversationHandler.END
    ...
    if cb.startswith("ai|") and not is_admin(uid):
        ...
        if action == "faq":
            ...
            await query.edit_message_text(
                "\n".join(lines)[:4000],
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(L(uid,"close"), callback_data="close")]]))
        return ConversationHandler.END
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_main_admin(uid): return

    # Spam koruması
    if not is_admin(uid) and not check_rate_limit(uid):
        await msg.reply_text(L(uid,"spam_warning")); return

    if text:
        log_user_message(user, "msg", text)

    if not text or is_admin(uid): return

    # Sabit mesaj varsa göster (sadece ilk mesajda)
    pinned = get_pinned_msg()
    if pinned and context.user_data.get("shown_pinned") is None:
        context.user_data["shown_pinned"] = True
        await msg.reply_text(L(uid,"pinned_msg_label").format(pinned))

    # ── Akıllı AI yanıt motoru ──────────────────────────────
    subject  = _detect_subject(text)
    is_q     = _is_question(text)

    # Konu tespit edildiyse kullanıcıya bildir
    subj_label = _subject_label(uid, subject)
    typing_msg = None
    if (subject or is_q) and len(text) > 4:
        try:
            typing_msg = await msg.reply_text(
        ...
        if answer:
            ...
            # Kaynak tipine göre format
            if source == "calc":
                reply_text = L(uid,"ai_math_result").format(result=ans_text)
            elif source == "rule" or source == "faq":
                reply_text = ans_text
            ...
            # Geri bildirim butonu ekle
            file_key = text[:60]
            kb = [[
                InlineKeyboardButton("⭐1", callback_data=f"rate|{file_key[:50]}|1"),
                InlineKeyboardButton("⭐2", callback_data=f"rate|{file_key[:50]}|2"),
                InlineKeyboardButton("⭐3", callback_data=f"rate|{file_key[:50]}|3"),
                InlineKeyboardButton("⭐4", callback_data=f"rate|{file_key[:50]}|4"),
                InlineKeyboardButton("⭐5", callback_data=f"rate|{file_key[:50]}|5"),
            ]]
            try:
                await msg.reply_text(reply_text, parse_mode="Markdown",
                                     reply_markup=InlineKeyboardMarkup(kb))
            except Exception:
                await msg.reply_text(reply_text, reply_markup=InlineKeyboardMarkup(kb))

            # Liderboard puan ekle
            update_leaderboard(uid, 1)
            # Başarı kontrol
            m_d = load_messages()
            user_msgs = m_d.get(uid, [])
            view_count = sum(1 for m in user_msgs if m.get("type") == "file_view")
            fav_count  = len(get_favorites(uid))
            new_achs   = check_achievements(uid, {"views": view_count, "favs": fav_count})
            for ach in new_achs:
                try: await msg.reply_text(L(uid,"achievement_unlocked").format(ach))
                except: pass
            return

        # Yanıt bulunamadı — FAQ butonu öner
        faqs = load_faq()
        if faqs:
            kb = [[InlineKeyboardButton(L(uid,"ai_faq_btn"), callback_data="ai|faq")]]
            await msg.reply_text(L(uid,"ai_no_web_result"), reply_markup=InlineKeyboardMarkup(kb))
        else:
            await msg.reply_text(L(uid,"ai_no_answer"))
    else:
        # Konu tespit edilemedi, FAQ öner
        faqs = load_faq()
        if faqs:
            kb = [[InlineKeyboardButton(L(uid,"ai_faq_btn"), callback_data="ai|faq")]]
            await msg.reply_text(L(uid,"ai_no_answer"), reply_markup=InlineKeyboardMarkup(kb))
...
def main():
    ...
    # ConversationHandler her şeyi yakalar (reply butonları fallback'te)
