...
TR = {
    ...
    "btn_anon":       "✉️ رسالة مجهولة",
    "anon_prompt":    "✉️ اكتب رسالتك المجهولة:",
    "anon_sent":      "✅ تم إرسال رسالتك بشكل مجهول.",
    "anon_disabled":  "❌ هذه الميزة غير مفعلة حالياً.",
...
ALL_BTNS = {
    ...
    TR.get("btn_anon","\u2709\ufe0f Anonim Mesaj"), AR.get("btn_anon","\u2709\ufe0f \u0631\u0633\u0627\u0644\u0629 \u0645\u062c\u0647\u0648\u0644\u0629"),
    "\U0001f52c \u0627\u0644\u0645\u062e\u062a\u0628\u0631",
    TR.get("btn_anon","✉️ Anonim Mesaj"), AR.get("btn_anon","✉️ رسالة مجهولة"),
    "🔬 المختبر",
}
...
def reply_kb(uid):
    ...
    if is_main_admin(uid):
        ...
        row5.append(KeyboardButton(AR["btn_anon"]))
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔬 المختبر" and not is_admin(uid):
        ...
        # Kullanıcının grubuna ait gelecekteki lab kayıtları
        my_labs = []
        all_labs = []
        for e in schedule:
            try:
                e_dt = _dt.strptime(e["week"], "%Y-%m-%d")
                if e_dt.date() >= today_ts.date():
                    grp = e.get("group","")
                    note = e.get("note","")
                    date_disp = e_dt.strftime("%d/%m/%Y")
                    is_mine = user_grp and grp == user_grp
                    all_labs.append((date_disp, grp, note, is_mine))
                    if is_mine:
                        my_labs.append((date_disp, grp, note))
            except: pass

        if not all_labs:
            await update.message.reply_text("🔬 المختبر\n\nلا يوجد جدول للمختبر قريباً.")
            return

        lines = ["🔬 جدول المختبر\n"]
        if my_labs:
            lines.append("📌 مواعيدك:")
            for date_disp, grp, note in my_labs[:5]:
                nt = f" — {note}" if note else ""
                lines.append(f"  ✅ {date_disp}  {grp}{nt}")
            lines.append("")
        lines.append("📅 الجدول الكامل:")
        for date_disp, grp, note, is_mine in all_labs[:10]:
            nt = f" — {note}" if note else ""
            icon = "👉" if is_mine else "  •"
            lines.append(f"  {icon} {date_disp}:  {grp}{nt}")
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("lab|") and is_main_admin(uid):
        ...
        if lab_act == "set":
            ...
            await query.answer(f"✅ {week} — {group} — {lab_name}", show_alert=True)
            query.data = "mgmt|lab"
            return await callback(update, context)
    ...
    if cb.startswith("cd_grp|") and is_admin(uid):
        grp_val = cb.split("|")[1]
...
def main():
    ...
    async def _check_lab_reminders(ctx):
        """Laboratuvar gününden 1 gün önce grubu bildir."""
