...
def build_target_keyboard(prefix: str, step: str, val: str = "") -> list:
    ...
    if step == "grp":
        kb = [
            [InlineKeyboardButton("🌐 الكل", callback_data=f"{base}grp_all|subgrp"),
             InlineKeyboardButton("A",        callback_data=f"{base}grp_A|subgrp"),
             InlineKeyboardButton("B",        callback_data=f"{base}grp_B|subgrp"),
             InlineKeyboardButton("C",        callback_data=f"{base}grp_C|subgrp")],
        ]
        return kb
    ...
    if step == "subgrp":
        ...
        for part in val.split("|"):
            if part.startswith("grp_") and part != "grp_all":
                grp_val = part[4:]  # A, B, C
        if grp_val:
            kb = [
                [InlineKeyboardButton("الكل", callback_data=f"{base}sub_all|done"),
                 InlineKeyboardButton(f"{grp_val}1", callback_data=f"{base}sub_{grp_val}1|done"),
                 InlineKeyboardButton(f"{grp_val}2", callback_data=f"{base}sub_{grp_val}2|done"),
                 InlineKeyboardButton(f"{grp_val}3", callback_data=f"{base}sub_{grp_val}3|done")],
            ]
        else:
            kb = [[InlineKeyboardButton("الكل", callback_data=f"{base}sub_all|done")]]
        return kb
    ...
    if text in (TR["btn_content"], AR["btn_content"]):
        if is_main_admin(uid):
            ...
            if get_admin_perm(uid, "can_countdown"):
                kb.append([InlineKeyboardButton("اضافة امتحان", callback_data="cnt|add_countdown")])
            if get_admin_perm(uid, "can_poll"):
                kb.append([InlineKeyboardButton("انشاء استطلاع", callback_data="poll|create")])
            if get_admin_perm(uid, "can_broadcast"):
                kb.append([InlineKeyboardButton("ارسال اعلان", callback_data="admin_bcast|panel")])
            if not kb:
                await update.message.reply_text("لا توجد صلاحيات.")
                return
            kb.append([InlineKeyboardButton(L(uid,"back"), callback_data="nav|root")])
            sent = await update.message.reply_text(L(uid,"content_mgmt"), reply_markup=InlineKeyboardMarkup(kb))
            context.user_data["last_inline_msg"] = sent.message_id
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("user|") and is_main_admin(uid):
        ...
        if action == "export":
            ...
            def clean(s):
                """Metni temizle, unicode koru (Arapça dahil)."""
                if not s: return "-"
                try:
                    val = str(s).strip()
                    # Emoji ve özel kontrol karakterlerini temizle (sadece)
                    import unicodedata
                    cleaned = "".join(ch for ch in val
                        if unicodedata.category(ch) not in ("Cc", "Cf")
                        or ch in ("\n", "\t"))
                    return cleaned.strip() or "-"
                except:
                    return str(s)[:200] if s else "-"
            to_ascii = clean
            ...
            info_rows = [
                ...
                ("Sinif",    cls_str),
                ("Grup",     grp_str),
                ("Vardiya",  clean(get_user_shift(target))),
                ("Son Giris",u.get("last_seen","-")),
                ("Rapor",    datetime.now().strftime("%Y-%m-%d %H:%M")),
                ("Toplam Aktivite", len(msgs)),
            ]
            for key, val in info_rows:
                row = ws1.max_row + 1
                ws1.cell(row, 1, key).font = Font(bold=True)
                ws1.cell(row, 2, str(val) if val else "-")
            ...
            if notes_data:
                ...
                ws4.append(["Ders", "Tur", "Icerik", "Zaman"])
                for col in range(1, 5):
    ...
    if cb.startswith("poll|") and is_main_admin(uid):
        ...
        if action == "create":
            # Adım adım hedefleme: Sınıf → Vardiya → Grup → Alt Grup
