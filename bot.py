...
def load_content():
    ...
    if TEMP_FOLDER_NAME not in data["folders"]:
        data["folders"][TEMP_FOLDER_NAME] = {
            "folders": {"Sabahci": {"folders": {}, "files": []},
                        "Gececi":  {"folders": {}, "files": []}},
            "files": []
        }
        changed = True
    if changed:
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in (TR["profile_btn"], AR["profile_btn"]):
        if not is_admin(uid):
            ...
            shift_lbl = "Sabahci" if shift == "sabah" else ("Gececi" if shift == "gece" else "—")
            kb = [
    ...
    if text in (TR["countdown_btn"], AR["countdown_btn"]):
        ...
        # Kendi sınıfına özel + herkese açık sınavlar
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("class_pick|"):
        ...
        if cls in CLASS_DEFS:
            ...
            kb_shift = [
                [InlineKeyboardButton("Sabahci", callback_data="shift_pick|sabah"),
                 InlineKeyboardButton("Gececi",  callback_data="shift_pick|gece")],
            ]
    ...
    if cb.startswith("shift_pick|") and not is_admin(uid):
        ...
        shift_lbl = "Sabahci" if shift == "sabah" else "Gececi"
        kb_grp = [
    ...
    if cb.startswith("set|") and is_main_admin(uid):
        ...
        if action == "group_mgmt":
            ...
            sab = sum(1 for v in shft.values() if v == "sabah")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "countdown_date" and is_admin(uid):
        ...
        ok = add_countdown(name, text, cls=admin_cls or "")
        if ok:
