...
TR = {
    ...
    "empty":"📭 Henüz içerik yok","back":"◀️ Geri","home_btn":"🏠 Ana",
...
AR = {
    ...
    "empty":"📭 لا يوجد محتوى بعد","back":"◀️ رجوع","home_btn":"🏠 الرئيسية",
...
(WAIT_FOLDER, WAIT_DEL_FOLDER, WAIT_FILE, WAIT_DEL_FILE,
 ...
 WAIT_DM_USER_ID, WAIT_DM_MSG, WAIT_BROADCAST_MSG) = range(13)
...
def load_settings():
    default = {
        ...
        "maintenance_text_ar": "🔧 البوت قيد التحديث...",
...
async def get_ai_response(user_id: str, message: str, lang: str = "ar") -> str:
    ...
    uid_history = uid_history[-20:]

    if lang == "tr":
        sys_prompt = (
            "Sen bu botun yardımcı yapay zeka asistanısın. Türkçe konuş. "
            "Kullanıcılara yardımcı ol ve sorularını yanıtla. "
        ...
        sys_prompt = (
            "أنت مساعد ذكاء اصطناعي مفيد لهذا البوت. تحدث باللغة العربية. "
            "ساعد المستخدمين وأجب على أسئلتهم. "
            "إذا طلب المستخدم شيئاً خاصاً أو تقنياً أو عاجلاً "
            "(دفع، عملية خاصة، شكوى، أو موضوع لا يمكن حله عبر البوت)، "
            "أضف [ADMIN_TALEP] في نهاية ردك مع ملخص قصير للطلب. "
            "لا تضف [ADMIN_TALEP] للمحادثة العادية."
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json={"model": "claude-haiku-4-5-20251001", "max_tokens": 800,
                      "system": sys_prompt, "messages": uid_history},
...
async def handle_ai_message(msg, context, uid: str, text: str):
    """AI mesajını işle ve yanıtla — her iki modda da kullanılır"""
    s = load_settings()
...
def folder_kb(path, folder, uid):
    ...
    if path:
        ...
        nav.append(InlineKeyboardButton(L(uid, "home_btn"), callback_data="nav|root"))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Klavyeyi kur — NOKTASIZ, sadece emoji
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    folder  = get_folder(content,path); action = context.user_data.get("action","")
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text and s.get("ai_enabled", True):
        ...
        # Her iki modda da metin mesajları AI'ya gider
        # (chat modunda zaten bekleniyor, browse modunda da yardımcı olur)
        await context.bot.send_chat_action(msg.chat_id, "typing")
        lang     = "tr" if is_main_admin(uid) else "ar"
        ai_reply = await get_ai_response(uid, text, lang)
        if "[ADMIN_TALEP]" in ai_reply:
...
def main():
    ...
    # Tüm buton metinlerinden regex oluştur (özel karakterleri escape et)
    import re
    ...
    print(f"🤖 Yapay Zeka     : {'✅ Aktif' if ANTHROPIC_API_KEY else '⚠️ Pasif (ANTHROPIC_API_KEY eksik)'}")
    if "/data" in BASE_DIR:
