...
DEFAULT_ADMIN_PERMS = {
    "can_warn":      True,
    "can_block":     False,   # False = süper admin onayı gerekli
    "can_broadcast": True,
    "can_add_folder":True,
    "can_add_file":  True,
    "can_del_folder":False,
    "can_del_file":  False,
    "can_poll":      True,
    "can_quiz":      False,
    "cls":           None,    # None = tüm sınıflar, "1"/"2"/"3"/"4" = sadece o sınıf
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cls:
        ...
        cls_ar_norm = _normalize_ar(cls_ar)
        for fname in content.get("folders", {}):
            if _normalize_ar(fname) == cls_ar_norm:
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb == "group_pick_start" and not is_admin(uid):
        ...
        await query.edit_message_text(L(uid,"group_select"), reply_markup=InlineKeyboardMarkup(kb_grp))
        return ConversationHandler.END
    ...
    if cb.startswith("adminperm|") and is_main_admin(uid):
        ...
        if act_p == "show":
            ...
            cls_lbl = f"Sınıf: {cls_val or 'Hepsi'}"
            kb.append([InlineKeyboardButton(f"🎓 {cls_lbl}", callback_data=f"adminperm|cls|{adm_id}")])
            kb.append([InlineKeyboardButton("◀️ Geri", callback_data="set|admin_perms")])
            await query.edit_message_text(
                f"👮 {name}\n{'─'*22}\nYetkilerini düzenle:",
                reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
        ...
        if act_p == "cls":
            ...
            kb = [
                ...
                [InlineKeyboardButton("1️⃣", callback_data=f"adminperm|setcls|{adm_id}|1"),
                 InlineKeyboardButton("2️⃣", callback_data=f"adminperm|setcls|{adm_id}|2"),
                 InlineKeyboardButton("3️⃣", callback_data=f"adminperm|setcls|{adm_id}|3"),
                 InlineKeyboardButton("4️⃣", callback_data=f"adminperm|setcls|{adm_id}|4")],
            ]
            await query.edit_message_text("Hangi sınıfı yönetsin?", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
    ...
    if cb.startswith("approve|") and is_main_admin(uid):
        ...
        if action == "block":
            ...
            # Kullanıcıya bildir
            try:
                await context.bot.send_message(int(target), AR["blocked_notify"] if AR.get("blocked_notify") else "🚫 تم حظرك من البوت.")
            except: pass
    ...
    if cb.startswith("user|") and is_main_admin(uid):
        ...
        if action == "export":
            ...
            lines = [
                ...
                f"Sınıf: {class_label(target)}",
                f"Son görülme: {u.get('last_seen','—')}",
                f"",
                f"📋 TÜM AKTİVİTE KAYITLARI",
                f"{'─'*28}",
            ]
            # Tüm mesajlar
            for m in msgs:
                icon = {"msg":"✍️","photo":"🖼","video":"🎥","document":"📄",
                        "voice":"🎙","button":"🔘","search":"🔍","file_view":"👁",
                        "command":"⚙️"}.get(m.get("type",""),"📌")
                lines.append(f"{m.get('time','')}  {icon}  {m.get('content','')[:80]}")
            # Hatırlatıcılar
            rems = get_user_reminders(target)
            if rems:
                lines.append(f"\n⏰ HATIRLATICILARI ({len(rems)} adet)")
                lines.append("─"*28)
                import time
                for r in rems:
                    left = max(0, int((r.get("fire_ts",0) - time.time()) / 60))
                    lines.append(f"• {r.get('text','')} — {left} dk kaldı")
            # Notlar
            notes = get_user_notes(target)
            if notes:
                lines.append(f"\n📝 NOTLARI ({len(notes)} adet)")
                lines.append("─"*28)
                for n in notes:
                    lines.append(f"• [{n.get('time','')}] {n.get('type','')} — {n.get('content','')[:60]}")

            report_text = "\n".join(lines)
            # Dosya olarak gönder
            import io
            bio = io.BytesIO(report_text.encode("utf-8"))
            bio.name = f"user_{target}_report.txt"
            bio.seek(0)
            try:
                await query.message.reply_document(
                    bio,
                    filename=f"user_{target}_report.txt",
                    caption=f"📥 {name} — Aktivite Raporu")
            except Exception as e:
        ...
        if action == "warn":
            ...
            if not is_main_admin(uid):
                # Alt admin → süper admin'e onay gönder
                await context.bot.send_message(
                    ADMIN_ID,
                    f"⚠️ UYARI ONAY TALEBİ\n{'─'*22}\n"
                    f"👮 Admin: ID {uid}\n"
                    f"👤 Hedef: {name} ({target})\n"
                    f"📋 İşlem: Uyarı Ver",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ Onayla + Uyarı Yaz", callback_data=f"approve|warn|{target}|{uid}"),
                        InlineKeyboardButton("❌ Reddet", callback_data=f"approve|deny|{target}|{uid}|warn"),
                    ]]))
                await query.answer("✅ Süper admin onayına gönderildi.", show_alert=True)
                return ConversationHandler.END
            context.user_data["warn_target"] = target
            context.user_data["action"]      = "warn_reason"
            await query.edit_message_text(
        ...
        if action == "block":
            ...
            if not is_main_admin(uid):
                await context.bot.send_message(
                    ADMIN_ID,
                    f"🚫 ENGELLEME ONAY TALEBİ\n{'─'*22}\n"
                    f"👮 Admin: ID {uid}\n"
                    f"👤 Hedef: {name} ({target})\n"
                    f"📋 İşlem: Engelle",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("✅ Engelle", callback_data=f"approve|block|{target}|{uid}"),
                        InlineKeyboardButton("❌ Reddet",  callback_data=f"approve|deny|{target}|{uid}|block"),
                    ]]))
                await query.answer("✅ Süper admin onayına gönderildi.", show_alert=True)
                return ConversationHandler.END
            blocked=load_blocked()
    ...
    if cb.startswith("poll|") and is_main_admin(uid):
        ...
        if action == "create":
            ...
            kb = [
                ...
                [InlineKeyboardButton("1️⃣ الأول",   callback_data="poll|target|cls_1"),
                 InlineKeyboardButton("2️⃣ الثاني",  callback_data="poll|target|cls_2")],
                [InlineKeyboardButton("3️⃣ الثالث",  callback_data="poll|target|cls_3"),
                 InlineKeyboardButton("4️⃣ الرابع",  callback_data="poll|target|cls_4")],
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "reminder_day" and not is_admin(uid):
        try: day = int(text.strip()); assert 1 <= day <= 31
        except: await update.message.reply_text("❌ 1-31"); return WAIT_FOLDER
        context.user_data["rem_day"] = day
        context.user_data["action"] = "reminder_hour"
        await update.message.reply_text("🕐 الساعة (0-23):" if not is_main_admin(uid) else "🕐 Saat (0-23):")
        return WAIT_FOLDER

    if action == "reminder_hour" and not is_admin(uid):
        try: hour = int(text.strip()); assert 0 <= hour <= 23
        except: await update.message.reply_text("❌ 0-23"); return WAIT_FOLDER
        context.user_data["rem_hour"] = hour
        context.user_data["action"] = "reminder_minute"
        await update.message.reply_text("⏱ الدقيقة (0-59):" if not is_main_admin(uid) else "⏱ Dakika (0-59):")
        return WAIT_FOLDER

    if action == "reminder_minute" and not is_admin(uid):
        try: minute = int(text.strip()); assert 0 <= minute <= 59
        except: await update.message.reply_text("❌ 0-59"); return WAIT_FOLDER
        ...
        try:
            target = _dt(now.year, now.month, day, hour, minute)
            if target < now:
                m = now.month + 1 if now.month < 12 else 1
                y = now.year if now.month < 12 else now.year + 1
                target = _dt(y, m, day, hour, minute)
        except Exception as e:
            await update.message.reply_text(f"❌ {e}"); return WAIT_FOLDER
        rem_text = context.user_data.pop("reminder_text_pending", "?")
        add_reminder(uid, rem_text, target.timestamp())
        await update.message.reply_text(L(uid,"reminder_saved").format(target.strftime("%d/%m %H:%M")))
        context.user_data.pop("action", None)
        return ConversationHandler.END
    ...
    if action == "user_msg_to_admin" and not is_admin(uid):
        ...
        cls_name = CLASS_DEFS.get(cls_id, {}).get("ar","") if cls_id else ""
        notif   = (
            f"💬 رسالة جديدة\n{'─'*22}\n"
            f"👤 {name}{un}\n"
            f"🆔 {uid}\n"
        ...
        kb_admin = [[InlineKeyboardButton(f"💬 رد", callback_data=f"dm_quick|{uid}")]]
        # Süper admin'e gönder
        try:
            await context.bot.send_message(ADMIN_ID, notif, reply_markup=InlineKeyboardMarkup(kb_admin))
        except Exception as e:
            logger.error(f"Admin bildirim hatası: {e}")
        # Sınıf adminine de gönder (varsa)
        if cls_id:
            perms_data = load_admin_perms()
            for adm_id_str, adm_perms in perms_data.items():
                adm_cls = adm_perms.get("cls")
                if adm_cls == cls_id and adm_id_str != str(ADMIN_ID):
                    try:
                        await context.bot.send_message(
                            int(adm_id_str), notif, reply_markup=InlineKeyboardMarkup(kb_admin))
                    except: pass
