...
def reply_kb(uid):
    ...
    if is_main_admin(uid):
        ...
        if load_settings().get("anon_group_id"):
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in (TR.get("btn_anon",""), AR.get("btn_anon","")) and not is_admin(uid):
        s = load_settings()
        if not s.get("anon_group_id"):
            await update.message.reply_text(L(uid,"anon_disabled"))
            return
        context.user_data["action"] = "anon_msg"
        kb = [[InlineKeyboardButton("◀️ " + L(uid,"back"), callback_data="nav|root")]]
        sent = await update.message.reply_text(L(uid,"anon_prompt"), reply_markup=InlineKeyboardMarkup(kb))
        context.user_data["last_inline_msg"] = sent.message_id
        return
...
def main():
    ...
    # Mevcut webhook varsa sil (Railway'de polling çakışmasını önle)
    import asyncio
    async def _clear_webhook():
        await app.bot.delete_webhook(drop_pending_updates=True)
        logger.info("✅ Webhook temizlendi, polling başlıyor...")
    asyncio.get_event_loop().run_until_complete(_clear_webhook())
