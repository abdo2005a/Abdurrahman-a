...
TR = {
    ...
    "set_ai_btn":         "🤖 Yapay Zeka Aç/Kapat",
    ...
    "ai_toggle_on":       "🤖 Yapay Zeka: ✅ AÇIK",
    "ai_toggle_off":      "🤖 Yapay Zeka: ❌ KAPALI",
    "blocked_list":       "🚫 Engellenenler ({})",
    "no_blocked":         "Kimse engellenmedi.",
    "maint_toggle":       "Bakım Modu: {}",
    "maint_on_str":       "AÇIK 🔧",
    "maint_off_str":      "KAPALI ✅",
    "search_prompt":  "🔍 Arama yapmak istediğin kelimeyi yaz:",
    "search_results": "🔍 '{}' için sonuçlar:",
    "search_none":    "❌ '{}' için sonuç bulunamadı.",
    "help_text": (
        "❓ *YARDIM — Süper Admin*\n\n"
        "📁 *İçerik* — Klasör ve dosya ekle/sil/düzenle\n"
        "👥 *Yönetim* — Kullanıcılar, adminler, duyuru\n"
        "⚙️ *Ayarlar* — Bot adı, karşılama, fotoğraf, AI\n"
        "🔧 *Bakım* — Bakım modunu aç/kapat\n"
        "🤖 *AI Sohbet* — Yapay zeka ile sohbet\n"
        "🔍 *Ara* — Klasör ve dosyalarda ara\n"
        "📊 *İstatistik* — Bot istatistikleri\n\n"
        "Eklediğin adminler sadece İçerik ve Bakım'a erişebilir."
    ),
...
AR = {
    ...
    "help_text": (
        ...
        "📊 *إحصائياتي* — شاهد إحصائياتك\n\n"
        "💡 استخدم *محادثة AI* لأي سؤال هندسي أو دراسي!\n"
        "إذا لم تجد ما تبحث عنه، أخبر المساعد وسيُحيلك للمسؤول."
    ),
...
def reply_kb(uid):
    ...
    if is_main_admin(uid):
        ...
        return ReplyKeyboardMarkup([
            ...
            [KeyboardButton(TR["btn_mystats"]),  KeyboardButton(TR["btn_help"])],
        ], resize_keyboard=True)
    elif is_admin(uid):
        # Diğer adminler — Arapça, sadece içerik + bakım + arama + yardım
        return ReplyKeyboardMarkup([
            [KeyboardButton(AR["btn_content"]),  KeyboardButton(AR["btn_maint"])],
            [KeyboardButton(AR["btn_search"]),   KeyboardButton(AR["btn_help"])],
        ], resize_keyboard=True)
    else:
        # Kullanıcılar — Arapça, sadece AI + arama + istatistik + yardım
        return ReplyKeyboardMarkup([
            [KeyboardButton(AR["btn_chat"]),     KeyboardButton(AR["btn_search"])],
            [KeyboardButton(AR["btn_mystats"]),  KeyboardButton(AR["btn_help"])],
        ], resize_keyboard=True)
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── İstatistik (herkes, içerik farklı) ──────────
    if text in (TR["btn_mystats"], AR["btn_mystats"]):
        if is_main_admin(uid):
            ...
            txt = (
                ...
                f"🤖 AI: {'Açık' if s.get('ai_enabled', True) else 'Kapalı'}"
            )
        else:
            ...
            u_inf = load_users().get(uid, {})
            name  = u_inf.get("full_name") or u_inf.get("first_name") or f"ID:{uid}"
            txt = (
                f"📊 *{AR['mystats_title']}*\n\n"
                f"👤 {name}\n🆔 {uid}\n"
                f"🕐 {u_inf.get('last_seen', '—')}\n\n"
                f"💬 الرسائل: {len(texts)}\n"
                f"📎 الوسائط: {len(media)}\n"
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Kullanıcılar dosya gönderemez — sadece log
    if not is_admin(uid):
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not s.get("ai_enabled", True): return
