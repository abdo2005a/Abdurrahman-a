...
USER_AI_PREF_FILE = os.path.join(BASE_DIR, "user_ai_pref.json")  # kişisel AI tercihi
# =============================================================
...
TR = {
    ...
    "chat_mode_on":  "🤖 AI Sohbet modu açık. Mesajınızı yazın.\n/menu ile ana sayfaya dönebilirsiniz.",
    "ai_personal_on":  "🤖 AI Sohbet: ✅ AÇIK — artık mesajlarınıza yanıt vereceğim.",
    "ai_personal_off": "🤖 AI Sohbet: ❌ KAPALI — artık mesajlarınıza yanıt vermeyeceğim.",
    "ai_global_off": "🤖 Yapay zeka şu an sistem tarafından kapatılmış.",
    "ai_error":      "🤖 Şu an yanıt veremiyorum, lütfen sonra deneyin.",
    "admin_fwd":     "📨 Talebiniz yöneticiye iletildi.",
    ...
    "ai_toggle_on":       "🤖 Global AI: ✅ AÇIK",
    "ai_toggle_off":      "🤖 Global AI: ❌ KAPALI",
    ...
    "help_text": (
        "❓ *YARDIM — Süper Admin*\n\n"
        "📁 *İçerik* — Klasör/dosya ekle veya sil/düzenle\n"
        "👥 *Yönetim* — Kullanıcılar, adminler, duyuru\n"
        "⚙️ *Ayarlar* — Bot adı, karşılama, fotoğraf, global AI\n"
        "🔧 *Bakım* — Bakım modunu aç/kapat\n"
        "🤖 *AI Sohbet* — AI sohbet moduna geç\n"
        "🔍 *Ara* — Klasör ve dosyalarda ara\n"
        "🤖 *AI butonu* — Kişisel AI'ını aç/kapat\n\n"
        "Eklediğin adminler yalnızca klasör ve dosya ekleyebilir."
    ),
    "mystats_title": "📊 BOT İSTATİSTİKLERİ",
...
AR = {
    ...
    "chat_mode_on":  "🤖 وضع المحادثة مفعّل. اكتب رسالتك.\nاكتب /menu للعودة للقائمة.",
    "ai_personal_on":  "🤖 محادثة AI: ✅ مفعّلة — سأرد على رسائلك الآن.",
    "ai_personal_off": "🤖 محادثة AI: ❌ معطّلة — لن أرد على رسائلك.",
    "ai_global_off": "🤖 الذكاء الاصطناعي معطّل من قبل المسؤول حالياً.",
    "ai_error":      "🤖 لا أستطيع الرد الآن، يرجى المحاولة لاحقاً.",
    "admin_fwd":     "📨 تم إرسال طلبك إلى المسؤول.",
    ...
    "ai_toggle_on":       "🤖 AI العام: ✅ مفعّل",
    "ai_toggle_off":      "🤖 AI العام: ❌ معطّل",
    ...
    "help_text": (
        "❓ *المساعدة*\n\n"
        "📁 *القائمة* — تصفح المجلدات والملفات\n"
        "🤖 *محادثة AI* — تحدث مع المساعد الذكي\n"
        "🔍 *بحث* — ابحث عن ملف أو مجلد\n"
        "🤖 *زر AI* — شغّل أو أوقف المحادثة الذكية\n\n"
        "💡 استخدم *محادثة AI* لأي سؤال هندسي أو دراسي!"
    ),
...
def load_user_ai_pref(): return load_json(USER_AI_PREF_FILE, {})
def save_user_ai_pref(d): save_json(USER_AI_PREF_FILE, d)
...
def load_settings():
    default = {
        ...
        "bot_photo_id":     None,
        "ai_enabled":       True,   # Global AI aç/kapat (sadece süper admin)
        # Kullanıcılara gösterilecek butonlar (True=görünür, False=gizli)
        "user_buttons": {
            "btn_chat":     True,   # 🤖 محادثة AI
            "btn_search":   True,   # 🔍 بحث
            "btn_ai_toggle":True,   # 🤖 AI: toggle
            "btn_help":     True,   # 💬 راسل المسؤول
        },
    }
...
# Kişisel AI tercihi: True = açık (varsayılan), False = kapalı
def get_user_ai_pref(uid: str) -> bool:
    prefs = load_user_ai_pref()
    return prefs.get(str(uid), True)

def set_user_ai_pref(uid: str, value: bool):
    prefs = load_user_ai_pref()
    prefs[str(uid)] = value
    save_user_ai_pref(prefs)
...
(WAIT_FOLDER, WAIT_FILE,
 ...
 WAIT_SEARCH, WAIT_USER_MSG) = range(13)
...
# AI toggle buton metinleri dinamik olduğu için ALL_BTNS'e ekleyemeyiz
# Bunları handle_reply_buttons'da prefix ile yakalıyoruz
...
# ================================================================
#  YAPAY ZEKA (Groq — Llama 3.3 70B — Ücretsiz)
# ================================================================

AI_PROMPT_AR = (
    "أنت مساعد ذكاء اصطناعي متخصص في بوت 'مهندسي المستقبل' على تيليغرام. "
    "البوت مخصص لطلاب الهندسة ويحتوي على ملفات دراسية ومحاضرات ومراجع وموارد تعليمية. "
    "مهمتك: مساعدة الطلاب في أسئلتهم الهندسية والأكاديمية، شرح المفاهيم بأسلوب واضح، "
    "المساعدة في الرياضيات والفيزياء والبرمجة والمواد الهندسية. "
    "تحدث دائماً بالعربية. كن ودوداً وعلمياً ودقيقاً. "
    "إذا سأل المستخدم عن ملف معين، اقترح له استخدام زر البحث 🔍. "
    "إذا طلب شيئاً يحتاج تدخل المسؤول (إضافة محتوى، شكوى، طلب خاص)، "
    "أضف [ADMIN_TALEP] في نهاية ردك مع ملخص قصير. "
    "لا تضف [ADMIN_TALEP] للأسئلة العلمية العادية."
)

AI_PROMPT_TR = (
    "Sen mühendislik öğrencileri için tasarlanmış bir Telegram botunun yapay zeka asistanısın. "
    "Bot; ders dosyaları, notlar, referanslar ve teknik materyaller içeriyor. "
    "Kullanıcılara mühendislik, matematik, fizik, programlama konularında yardımcı ol. "
    "Türkçe konuş. Yönetici müdahalesi gereken taleplerde cevabının sonuna "
    "[ADMIN_TALEP] ve talebin özetini ekle. Normal sorular için ekleme."
)
...
async def get_ai_response(user_id: str, message: str, lang: str = "ar") -> str:
    ...
    sys_prompt  = AI_PROMPT_TR if lang == "tr" else AI_PROMPT_AR
...
async def notify_admin_request(context, user, summary: str):
    ...
    kb = [[InlineKeyboardButton(TR["reply_btn"].format(user.id),
                                callback_data=f"dm_quick|{user.id}")]]
    try:
...
def ai_btn_label(uid: str, lang_dict: dict) -> str:
    """Kişisel AI durumuna göre buton etiketi"""
    on = get_user_ai_pref(uid)
    return lang_dict["btn_ai_toggle"].format("✅ AÇIK" if on else "❌ KAPALI") \
        if lang_dict is TR else \
        lang_dict["btn_ai_toggle"].format("✅ مفعّل" if on else "❌ معطّل")

def reply_kb(uid):
    uid = str(uid)
    if is_main_admin(uid):
        ai_lbl = ai_btn_label(uid, TR)
        return ReplyKeyboardMarkup([
            [KeyboardButton(TR["btn_content"]),  KeyboardButton(TR["btn_mgmt"])],
            [KeyboardButton(TR["btn_settings"]), KeyboardButton(TR["btn_maint"])],
            [KeyboardButton(TR["btn_chat"]),     KeyboardButton(TR["btn_search"])],
            [KeyboardButton(ai_lbl),             KeyboardButton(TR["btn_help"])],
        ], resize_keyboard=True)
    elif is_admin(uid):
        # Diğer adminler: içerik ekle + bakım + arama + AI toggle + yardım
        ai_lbl = ai_btn_label(uid, AR)
        return ReplyKeyboardMarkup([
            [KeyboardButton(AR["btn_content"]),  KeyboardButton(AR["btn_maint"])],
            [KeyboardButton(AR["btn_chat"]),     KeyboardButton(AR["btn_search"])],
            [KeyboardButton(ai_lbl),             KeyboardButton(AR["btn_help"])],
        ], resize_keyboard=True)
    else:
        # Kullanıcılar: ayarda aktif olan butonları göster
        s        = load_settings()
        ub       = s.get("user_buttons", {})
        ai_lbl   = ai_btn_label(uid, AR)
        row1, row2 = [], []
        if ub.get("btn_chat",     True): row1.append(KeyboardButton(AR["btn_chat"]))
        if ub.get("btn_search",   True): row1.append(KeyboardButton(AR["btn_search"]))
        if ub.get("btn_ai_toggle",True): row2.append(KeyboardButton(ai_lbl))
        if ub.get("btn_help",     True): row2.append(KeyboardButton(AR["btn_help"]))
...
def folder_text(folder, path, uid):
    ...
    if not folds and not files:
        pass  # İçerik yoksa hiçbir şey gösterme
    if is_admin(uid):
        s = load_settings()
        lines.append("─" * 22)
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Kişisel AI Toggle — prefix ile yakala (herkes) ──
    # Buton "🤖 AI: ✅ AÇIK" veya "🤖 AI: ❌ KAPALI" şeklinde başlıyor
    if text.startswith("🤖 AI:"):
        current = get_user_ai_pref(uid)
        new_val  = not current
        set_user_ai_pref(uid, new_val)
        lang_dict = TR if is_main_admin(uid) else AR
        msg = L(uid, "ai_personal_on") if new_val else L(uid, "ai_personal_off")
        await update.message.reply_text(msg, reply_markup=reply_kb(uid))
        return

    # ── AI Sohbet modu (herkes) ──────────────────────
    if text in (TR["btn_chat"], AR["btn_chat"]):
        context.user_data["mode"] = "chat"
        await update.message.reply_text(L(uid, "chat_mode_on"))
        return
    ...
    # ── İçerik — Diğer adminler: sadece ekle ────────
    ...
    if text == TR["btn_settings"]:
        ...
        txt = (
            ...
            f"🌐 Global AI: {'✅ Açık' if s.get('ai_enabled',True) else '❌ Kapalı'}\n"
        ...
        kb = [
            ...
            [InlineKeyboardButton(TR["set_photo_btn"],   callback_data="set|photo"),
             InlineKeyboardButton(TR["set_ai_btn"],      callback_data="set|toggle_ai")],
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("dm_quick|") and is_main_admin(uid):
        target = cb.split("|")[1]
        context.user_data["dm_target"] = target; context.user_data["action"] = "dm_msg"
        await query.edit_message_text(TR["dm_enter_msg"].format(target),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(TR["cancel"], callback_data="close")]]))
        return WAIT_DM_MSG
    ...
    if cb.startswith("rem_admin|") and is_main_admin(uid):
        ...
        await query.edit_message_text(TR["admin_removed"].format(rem))
        return ConversationHandler.END
    ...
    if cb.startswith("user|") and is_main_admin(uid):
        ...
        if action == "info":
            ...
            lines  = [f"👤 {name} ({un})", f"🆔 {target}",
                      f"{TR['user_last']} {u.get('last_seen','—')}",
                      TR["user_msgs"].format(len(texts),len(media)), "─"*22]
            for m in msgs[-15:]:
                lines.append(f"{ICONS.get(m.get('type',''),'📨')} [{m.get('time','')[-8:]}] {m.get('content','')[:50]}")
            kb = []
            if media:
                kb.append([InlineKeyboardButton(TR["send_media_btn"].format(len(media)),
                                                callback_data=f"user|media|{target}")])
            kb.append([InlineKeyboardButton(TR["msg_btn"],   callback_data=f"dm_quick|{target}")])
            if int(target) not in [int(x) for x in load_blocked()]:
                kb.append([InlineKeyboardButton(TR["block_btn"],   callback_data=f"user|block|{target}")])
            else:
                kb.append([InlineKeyboardButton(TR["unblock_btn"], callback_data=f"user|unblock|{target}")])
            kb.append([InlineKeyboardButton(TR["back"], callback_data="mgmt|users")])
        ...
        if action == "media":
            ...
            if not media:
                await query.answer(TR["file_notfound"], show_alert=True); return ConversationHandler.END
    ...
    if cb.startswith("cnt|") and adm:
        ...
        if action == "del_file":
            ...
            kb = [[InlineKeyboardButton(f"🗑 {f.get('caption','?')}", callback_data=f"do|del_file|{i}")]
    ...
    if cb.startswith("set|") and is_main_admin(uid):
        ...
        if action == "toggle_ai":
            ...
            await query.edit_message_text(TR["ai_toggle_on"] if s["ai_enabled"] else TR["ai_toggle_off"])
            return ConversationHandler.END
        ...
        if action == "btn_mgmt":
            s  = load_settings()
            ub = s.get("user_buttons", {})
            # Her buton için açık/kapalı göster
            btn_defs = [
                ("btn_chat",     "🤖 AI Sohbet"),
                ("btn_search",   "🔍 Arama"),
                ("btn_ai_toggle","🤖 AI Toggle"),
                ("btn_help",     "💬 Mesaj Gönder"),
            ]
            kb = []
            for key, label in btn_defs:
                state = ub.get(key, True)
                icon  = "✅" if state else "❌"
                kb.append([InlineKeyboardButton(
                    f"{icon} {label}",
                    callback_data=f"set|toggle_btn|{key}"
                )])
            kb.append([InlineKeyboardButton(TR["close"], callback_data="close")])
            await query.edit_message_text(
        ...
        if action == "toggle_btn":
            key = cb.split("|")[2]
            s   = load_settings()
            ub  = s.setdefault("user_buttons", {
                "btn_chat": True, "btn_search": True,
                "btn_ai_toggle": True, "btn_help": True
            })
            ...
            for k, label in btn_defs:
                ...
                kb.append([InlineKeyboardButton(
                    f"{icon} {label}",
                    callback_data=f"set|toggle_btn|{k}"
                )])
            kb.append([InlineKeyboardButton(TR["close"], callback_data="close")])
            await query.edit_message_text(
                TR["btn_mgmt_title"] + "\n\n✅ = Kullanıcılar görebilir\n❌ = Kullanıcılar göremez",
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "user_msg_to_admin" and not is_admin(uid):
        ...
        un    = f" @{u['username']}" if u.get("username") else ""
        notif = L(str(ADMIN_ID), "admin_user_msg").format(
            name=name, un=un, uid=uid, text=text,
            time=datetime.now().strftime("%Y-%m-%d %H:%M"))
        kb = [[InlineKeyboardButton(TR["reply_btn"].format(uid), callback_data=f"dm_quick|{uid}")]]
        try:
            await context.bot.send_message(ADMIN_ID, notif, reply_markup=InlineKeyboardMarkup(kb))
        except Exception as e:
            logger.error(f"Admin bildirim hatası: {e}")
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "set_photo" and is_main_admin(uid):
        if msg.photo:
            ...
            await msg.reply_text(TR["set_photo_ok"]); context.user_data.pop("action",None)
        else:
            await msg.reply_text(TR["set_photo_fail"])
        return ConversationHandler.END
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # AI toggle buton metinlerini de atla
    ...
    # Admin için AI çalışmasın (sadece kullanıcılar için)
    if is_admin(uid): return

    # Arama bekleniyor mu?
    if context.user_data.get("action") == "search":
        context.user_data.pop("action",None)
        await do_search(msg, uid, text)
        return

    # Global AI kapalıysa herkese kapalı
    if not s.get("ai_enabled", True):
        await msg.reply_text(L(uid,"ai_global_off")); return

    # Kişisel AI tercihi
    if not get_user_ai_pref(uid): return  # kapalı → sessiz kal
...
def main():
    ...
    # AI toggle buton metinleri prefix tabanlı olduğu için ayrı yakalanıyor
    reply_btn_f = filters.Regex(f"^({'|'.join(escaped)})$")
    ai_toggle_f = filters.Regex(r"^🤖 AI:")

    # Kombine reply buton filtresi
    all_reply_f = reply_btn_f | ai_toggle_f

    conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(callback),
            MessageHandler(text_f & ~all_reply_f, handle_text),
        ],
        states={
            WAIT_FOLDER:        [MessageHandler(text_f & ~all_reply_f, handle_text)],
            WAIT_FILE:          [MessageHandler(text_f & ~all_reply_f, handle_text),
                                 MessageHandler(media_f, handle_media), CallbackQueryHandler(callback)],
            WAIT_ADMIN_ID:      [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_RENAME_FOLDER: [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_RENAME_FILE:   [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_BOT_NAME:      [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_WELCOME:       [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_PHOTO:         [MessageHandler(media_f, handle_media), CallbackQueryHandler(callback)],
            WAIT_DM_USER_ID:    [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_DM_MSG:        [MessageHandler(text_f & ~all_reply_f, handle_text),
                                 MessageHandler(media_f, handle_media), CallbackQueryHandler(callback)],
            WAIT_BROADCAST_MSG: [MessageHandler(text_f & ~all_reply_f, handle_text),
                                 MessageHandler(media_f, handle_media), CallbackQueryHandler(callback)],
            WAIT_SEARCH:        [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
            WAIT_USER_MSG:      [MessageHandler(text_f & ~all_reply_f, handle_text), CallbackQueryHandler(callback)],
        },
        fallbacks=[
            CommandHandler("start", start),
            CommandHandler("menu",  menu_command),
            MessageHandler(all_reply_f, handle_reply_buttons),
    ...
    app.add_handler(MessageHandler(all_reply_f, handle_reply_buttons))
    ...
    print(f"🤖 Groq AI      : {'✅ Aktif' if GROQ_API_KEY else '⚠️  KEY EKSİK → Railway Variables → GROQ_API_KEY'}")
    if "/data" in BASE_DIR:
