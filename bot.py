...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "note_subject_input" and not is_admin(uid):
        ...
        context.user_data["note_subject"] = subject
        context.user_data["action"] = "note_taking"  # Not alma modu
        lbl = subject if subject else "ملاحظة عامة"
        kb = [[InlineKeyboardButton("🔒 إنهاء الملاحظة", callback_data="notes|done")]]
        sent = await update.message.reply_text(
            f"📖 {lbl}\n─────\nأرسل ما تريد (نص، صورة، صوت، فيديو)\nاضغط 🔒 عند الانتهاء:",
            reply_markup=InlineKeyboardMarkup(kb))
        context.user_data["last_inline_msg"] = sent.message_id
        return WAIT_FILE  # Metin VE medya kabul et
    ...
    if action == "note_taking" and not is_admin(uid):
        ...
        return WAIT_FILE  # Devam et, kapatana kadar
