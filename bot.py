...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("msg_target|") and not is_admin(uid):
        ...
        return WAIT_FOLDER  # Metin girişi bekle
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "reminder_text" and not is_admin(uid):
        context.user_data["reminder_text_pending"] = text  # tutarlı key
        context.user_data.pop("action", None)
    ...
    if action == "reminder_day" and not is_admin(uid):
        ...
        try:
            day = int(t)
            assert 0 <= day <= 31  # 0 = bugün
        except: await update.message.reply_text("0-31 arasi bir sayi gir (0=bugun):"); return WAIT_FOLDER
        if day == 0:
