...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("admin_bcast|") and is_admin(uid) and not is_main_admin(uid):
        ...
        if ab_action == "panel":
            # Hedefleme: kendi sınıfına sınırlı ama içinde vardiya+grup seçimi
            if admin_cls:
                # Kendi sınıfı sabit — vardiya seç
                kb = [
                ...
                # Sınıf yok — tüm sınıflar + vardiya
                kb = build_target_keyboard("admin_bcast|tgt", "cls")
                kb.append([InlineKeyboardButton("◀️ إلغاء", callback_data="nav|root")])
                await query.edit_message_text("📢 السنة الدراسية:", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END

        if ab_action == "sft":
            sft = ab_action.split("|")[0] if "|" in ab_action else cb.split("|")[2]
            # Actually: cb = "admin_bcast|sft|sabahi"
            sft = cb.split("|")[2]
            context.user_data["admin_bcast_sft"] = "" if sft == "all" else sft
            admin_cls2 = get_admin_cls(uid)
            # Grup seç
            adm_grp2 = get_admin_grp(uid)
            if adm_grp2:
                # Grup sabit — direkt mesaj yaz
                context.user_data["admin_bcast_cls"] = admin_cls2
                context.user_data["admin_bcast_grp"] = adm_grp2
                ...
                return WAIT_FOLDER
            # Grup seç
            cls_grps = get_class_groups(admin_cls2 or "1")
            grp_keys = list(cls_grps.keys()) if admin_cls2 else ["A","B","C"]
            kb = [[InlineKeyboardButton("الكل", callback_data="admin_bcast|grp|all")]]
            row = []
            for g in grp_keys:
        ...
        if ab_action == "grp":
            grp = cb.split("|")[2]
            context.user_data["admin_bcast_grp"] = "" if grp == "all" else grp
            context.user_data["admin_bcast_cls"] = get_admin_cls(uid)
            context.user_data["action"] = "admin_bcast_msg"
            await query.edit_message_text("📢 اكتب نص الإعلان:",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ إلغاء","nav|root")]]))
            return WAIT_FOLDER
        ...
        if ab_action == "tgt":
            # build_target_keyboard flow
            parts_tg = cb.split("|")
            step_tg  = parts_tg[-1]
            val_tg   = "|".join(parts_tg[2:-1]) if len(parts_tg) > 3 else (parts_tg[2] if len(parts_tg) > 2 else "")
            if step_tg == "done":
                t_parsed = parse_target_val(val_tg)
                context.user_data["admin_bcast_cls"] = t_parsed.get("cls") or get_admin_cls(uid)
                context.user_data["admin_bcast_grp"] = t_parsed.get("grp")
                context.user_data["admin_bcast_sft"] = t_parsed.get("shift","")
                context.user_data["action"] = "admin_bcast_msg"
                await query.edit_message_text("📢 اكتب نص الإعلان:",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️","nav|root")]]))
                return WAIT_FOLDER
            else:
                kb = build_target_keyboard("admin_bcast|tgt", step_tg, val_tg)
                kb.append([InlineKeyboardButton("◀️","nav|root")])
                labels = {"cls":"السنة","shift":"الفترة","grp":"المجموعة"}
                await query.edit_message_text(f"📢 {labels.get(step_tg,step_tg)}:", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END

        if ab_action in ("send",) or ab_action.startswith("cls|"):
            tgt = get_admin_cls(uid) if ab_action == "send" else ab_action.split("|")[1]
    ...
    if cb.startswith("cd_shift|") and is_admin(uid) and (is_main_admin(uid) or get_admin_perm(uid, "can_countdown")):
        ...
        admin_cls = get_admin_cls(uid)
        if admin_cls:
            cls_grps = get_class_groups(admin_cls)
        ...
        row = []
        for g in grp_keys:
            row.append(InlineKeyboardButton(g, callback_data=f"cd_grp|{g}"))
        if row: kb.append(row)
        await query.edit_message_text(
            "للمجموعة:" if not is_main_admin(uid) else "Hangi grup?",
            reply_markup=InlineKeyboardMarkup(kb))
        return WAIT_FOLDER
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "admin_bcast_msg" and is_admin(uid) and not is_main_admin(uid):
        ...
        tgt_sft = context.user_data.pop("admin_bcast_sft", "")
        context.user_data.pop("action", None)
    ...
    if action == "lab_add_name" and is_main_admin(uid):
    ...
    if action == "lab_add_date" and is_main_admin(uid):
        date_str = text.strip()
        from datetime import datetime as _dt
        try:
        ...
        # Action kalsın ki WAIT_FOLDER'da callback işlensin
        context.user_data.pop("action", None)
        await update.message.reply_text(
            f"📅 {date_str}  🔬 {lab_name}\nHangi grup giriyor?",
            reply_markup=InlineKeyboardMarkup(kb))
    ...
    if action == "countdown_date" and is_admin(uid):
        ...
        ok = add_countdown(name, text, cls=admin_cls or "", shift=shift_val, group=grp_val)
        if ok:
