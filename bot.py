...
(WAIT_FOLDER, WAIT_DEL_FOLDER, WAIT_FILE, WAIT_DEL_FILE,
 ...
 WAIT_DM_USER_ID, WAIT_DM_MSG,
 WAIT_BROADCAST_MSG) = range(13)
...
# ═══════════════════════════════════════════════════════
#  YAPAY ZEKA (Anthropic Claude)
# ═══════════════════════════════════════════════════════

async def get_ai_response(user_id: str, user_name: str, message: str) -> str:
    """Claude API'ye istek gönder, konuşma geçmişiyle"""
    import aiohttp

    if not ANTHROPIC_API_KEY:
        return "⚠️ Yapay zeka şu an aktif değil. Lütfen yöneticiye ulaşın."
    ...
    uid_history = uid_history[-20:]  # Son 20 mesaj

    system_prompt = (
        f"Sen bu botun yardımcı yapay zeka asistanısın. "
        f"Kullanıcılarla Türkçe veya Arapça (kullanıcının diline göre) konuş. "
        f"Kullanıcılara yardımcı ol, sorularını yanıtla. "
        f"Eğer kullanıcı çok özel, teknik veya acil bir talep yaparsa (örneğin ödeme, "
        f"özel işlem, şikayet veya bot ile çözülemeyen bir konu), cevabının sonuna "
        f"mutlaka şunu ekle: [ADMIN_TALEP] ve talebi kısaca özetle. "
        f"Normal sohbet ve genel yardım için [ADMIN_TALEP] ekleme."
    )
    ...
    try:
        ...
        return "🤖 Şu an yanıt veremiyorum, lütfen biraz sonra tekrar deneyin."

async def notify_admin_request(context, user, summary: str):
    """Kullanıcının özel talebini süper admine bildir"""
    users = load_users()
    ...
    text = (
        f"🔔 YENİ KULLANICI TALEBİ\n\n"
        f"👤 {name}{username}\n"
        f"🆔 {user.id}\n"
        f"📋 Talep: {summary}\n"
        f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
    kb = [[InlineKeyboardButton(f"💬 Yanıtla (ID: {user.id})", callback_data=f"dm_quick|{user.id}")]]
    try:
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == MGMT_BTN:
        ...
        if is_main_admin(uid):
            ...
            kb.append([
                InlineKeyboardButton("💬 Kullanıcıya Mesaj", callback_data="mgmt|dm_user"),
                InlineKeyboardButton("📢 Herkese Duyuru", callback_data="mgmt|broadcast"),
            ])
    ...
    if text == SETTINGS_BTN and is_main_admin(uid):
        ...
        kb = [
            ...
            [InlineKeyboardButton("🤖 Yapay Zeka Aç/Kapat", callback_data="set|toggle_ai")],
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("mgmt|") and adm:
        ...
        # ── Özel Kullanıcıya Mesaj ────────────────────
        if action == "dm_user" and is_main_admin(uid):
            context.user_data["action"] = "dm_user_id"
            u_d = load_users()
            # Kullanıcı listesi butonu olarak göster
            kb = []
            ...
            kb.append([InlineKeyboardButton("✏️ ID ile yaz", callback_data="dm_pick|manual")])
            kb.append([InlineKeyboardButton("❌ İptal", callback_data="close")])
            await query.edit_message_text("💬 Mesaj göndermek istediğin kullanıcıyı seç:", reply_markup=InlineKeyboardMarkup(kb))
            return ConversationHandler.END
    ...
    if cb.startswith("dm_pick|") and is_main_admin(uid):
        ...
        if target == "manual":
            ...
            context.user_data["action"]    = "dm_msg"
            u_d  = load_users()
            u    = u_d.get(target, {})
            name = u.get("full_name") or u.get("first_name") or f"ID:{target}"
            await query.edit_message_text(
                f"💬 {name} kişisine gönderilecek mesajı yaz:",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ İptal", callback_data="close")]]))
            return WAIT_DM_MSG

    # ── Hızlı DM (kullanıcı listesinden) ─────────────
    if cb.startswith("dm_quick|") and is_main_admin(uid):
        target = cb.split("|")[1]
        context.user_data["dm_target"] = target
        context.user_data["action"]    = "dm_msg"
        await query.edit_message_text(
            f"💬 {target} kişisine gönderilecek mesajı yaz:",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ İptal", callback_data="close")]]))
        return WAIT_DM_MSG
    ...
    if cb.startswith("user|") and adm:
        ...
        if action == "info":
            ...
            if is_main_admin(uid):
                kb.append([InlineKeyboardButton("💬 Mesaj Gönder", callback_data=f"dm_quick|{target}")])
    ...
    if cb.startswith("set|") and is_main_admin(uid):
        ...
        if action == "toggle_ai":
            ...
            save_settings(s)
            durum = "✅ AÇIK" if s["ai_enabled"] else "❌ KAPALI"
            await query.edit_message_text(f"🤖 Yapay Zeka: {durum}")
            return ConversationHandler.END
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "dm_manual_id" and is_main_admin(uid):
        ...
        except: await update.message.reply_text("❌ Geçersiz ID."); return WAIT_DM_USER_ID
        context.user_data["dm_target"] = target_id
        context.user_data["action"]    = "dm_msg"
        await update.message.reply_text(f"💬 {target_id} kişisine gönderilecek mesajı yaz:")
        return WAIT_DM_MSG
    ...
    if action == "dm_msg" and is_main_admin(uid):
        ...
        if not target:
            ...
            return ConversationHandler.END
        try:
            await context.bot.send_message(int(target), f"📩 Yönetici mesajı:\n\n{text}")
            await update.message.reply_text(f"✅ Mesaj gönderildi → {target}")
        except Exception as e:
    ...
    if action == "broadcast" and is_main_admin(uid):
        ...
        for uid_, u in users.items():
            ...
            try:
                await context.bot.send_message(int(uid_), f"📢 Duyuru:\n\n{text}")
                success += 1
            except Exception:
                fail += 1
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "broadcast" and is_main_admin(uid):
        ...
        for uid_, u in users.items():
            ...
            try:
                if msg.photo:
                    await context.bot.send_photo(int(uid_), msg.photo[-1].file_id, caption=caption_text)
                elif msg.video:
                    await context.bot.send_video(int(uid_), msg.video.file_id, caption=caption_text)
                elif msg.document:
                    await context.bot.send_document(int(uid_), msg.document.file_id, caption=caption_text)
                elif msg.animation:
                    await context.bot.send_animation(int(uid_), msg.animation.file_id, caption=caption_text)
                success += 1
            except Exception:
                fail += 1
        await msg.reply_text(f"📢 Duyuru tamamlandı!\n✅ Gönderildi: {success}\n❌ Başarısız: {fail}")
        context.user_data.pop("action", None)
        return ConversationHandler.END
    ...
    if action == "dm_msg" and is_main_admin(uid):
        ...
        try:
            caption_text = (msg.caption or "") + "\n\n📩 Yönetici mesajı"
            if msg.photo:
                await context.bot.send_photo(int(target), msg.photo[-1].file_id, caption=caption_text)
            elif msg.video:
                await context.bot.send_video(int(target), msg.video.file_id, caption=caption_text)
            elif msg.document:
                await context.bot.send_document(int(target), msg.document.file_id, caption=caption_text)
            elif msg.voice:
                await context.bot.send_voice(int(target), msg.voice.file_id)
            await msg.reply_text(f"✅ Medya gönderildi → {target}")
        except Exception as e:
            await msg.reply_text(f"❌ Gönderilemedi: {e}")
        context.user_data.pop("action", None)
        context.user_data.pop("dm_target", None)
        return ConversationHandler.END
...
# ═══════════════════════════════════════════════════════
#  KULLANICI MESAJ LOGLAYICILARI (group=-1)
# ═══════════════════════════════════════════════════════

async def log_user_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Kullanıcı medyasını logla ve diske kaydet"""
...
# ═══════════════════════════════════════════════════════
#  GENEL MESAJ HANDLER (+ Yapay Zeka)
# ═══════════════════════════════════════════════════════
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text and s.get("ai_enabled", True):
        ...
        ai_reply = await get_ai_response(uid, user.first_name or "", text)

        # Admin talebini ayır ve bildir
        if "[ADMIN_TALEP]" in ai_reply:
            ...
            await msg.reply_text("📨 Talebiniz yöneticiye iletildi. En kısa sürede dönüş yapılacak.")
...
def main():
    ...
    conv = ConversationHandler(
        ...
        fallbacks=[
            ...
            CallbackQueryHandler(callback, pattern="^(close|nav\\||mgmt\\||cnt\\||set\\||user\\||do\\||open\\||getfile\\||rem_admin\\||dm_pick\\||dm_quick\\|)"),
        ],
        allow_reentry=True,
    )

    # group=-1: Her mesajdan ÖNCE çalışır
    ...
    print(f"🤖 Yapay Zeka     : {'✅ Aktif (ANTHROPIC_API_KEY bulundu)' if ANTHROPIC_API_KEY else '⚠️ Pasif (ANTHROPIC_API_KEY eksik)'}")
    if "/data" in BASE_DIR:
