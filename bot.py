...
# ================================================================
#  KULLANICI AKTİVİTESİNİ ADMİNE İLET — Detaylı & Estetik
# ================================================================

def _user_card(user) -> str:
    """Kullanıcı kimlik kartı — her bildirimde başlık olarak kullanılır."""
    users = load_users()
    ...
    msgs  = load_messages()
    total = len(msgs.get(str(user.id), []))
    return (
        f"╔══════════════════════╗\n"
        f"  👤  {name}\n"
        f"  🔗  {un}\n"
...
async def forward_to_admin(context, user, event_type: str, content: str = "",
                            file_id: str = None, ftype: str = None, extra: str = ""):
    """
    Kullanıcıdan gelen HER aktiviteyi gerçek zamanlı ve güzel biçimde admine ilet.
    event_type: 'text' | 'media' | 'button' | 'start' | 'command' | 'file_view' | 'search'
    """
    card = _user_card(user)
    kb   = [[
        InlineKeyboardButton("💬 Yanıtla", callback_data=f"dm_quick|{user.id}"),
        InlineKeyboardButton("👤 Profil",  callback_data=f"user|info|{user.id}"),
    ]]
    ...
    TYPE_LABEL = {
        ...
        "start":     "🚀  Bota Katıldı",
        "command":   "⚙️  Komut",
        "file_view": "👁️  Dosya Görüntüledi",
        "search":    "🔍  Arama Yaptı",
        "photo":     "🖼️  Fotoğraf Gönderdi",
        "video":     "🎥  Video Gönderdi",
        "voice":     "🎙️  Sesli Mesaj Gönderdi",
        "document":  "📄  Dosya Gönderdi",
        "audio":     "🎵  Ses Dosyası Gönderdi",
        "animation": "🎞️  GIF Gönderdi",
        "sticker":   "😊  Sticker Gönderdi",
        "video_note":"⭕  Yuvarlak Video Gönderdi",
    }
    label = TYPE_LABEL.get(ftype or event_type, "📨  Aktivite")
    ...
    try:
        if event_type == "text":
            ...
            await context.bot.send_message(ADMIN_ID, body,
            ...
            body = (
                f"{card}\n\n"
                f"┌─ {label}\n"
                f"│  Bot'a ilk kez veya yeniden başladı.\n"
                f"└──────────────────────"
            )
            await context.bot.send_message(ADMIN_ID, body,
                reply_markup=InlineKeyboardMarkup(kb))
        ...
        elif event_type == "media" and file_id:
            cap = (
                f"{card}\n\n"
                f"┌─ {label}\n"
                f"│  {('📝 Açıklama: ' + content) if content else '(Açıklama yok)'}\n"
                f"└──────────────────────"
            )
            if   ftype == "photo":      await context.bot.send_photo(    ADMIN_ID, file_id, caption=cap, reply_markup=InlineKeyboardMarkup(kb))
            elif ftype == "video":      await context.bot.send_video(    ADMIN_ID, file_id, caption=cap, reply_markup=InlineKeyboardMarkup(kb))
            elif ftype == "voice":      await context.bot.send_voice(    ADMIN_ID, file_id, caption=cap, reply_markup=InlineKeyboardMarkup(kb))
            elif ftype == "document":   await context.bot.send_document( ADMIN_ID, file_id, caption=cap, reply_markup=InlineKeyboardMarkup(kb))
            elif ftype == "audio":      await context.bot.send_audio(    ADMIN_ID, file_id, caption=cap, reply_markup=InlineKeyboardMarkup(kb))
            elif ftype == "animation":  await context.bot.send_animation(ADMIN_ID, file_id, caption=cap, reply_markup=InlineKeyboardMarkup(kb))
            elif ftype == "video_note": await context.bot.send_video_note(ADMIN_ID, file_id)
            elif ftype == "sticker":
                sticker_info = (
                    f"{card}\n\n"
                    f"┌─ {label}\n"
                ...
                await context.bot.send_message(ADMIN_ID, sticker_info,
                    reply_markup=InlineKeyboardMarkup(kb))

    except Exception as e:
        logger.error(f"forward_to_admin hata: {e}")

def make_poll_id():
    return datetime.now().strftime("%Y%m%d%H%M%S")

def poll_bar(count, total):
    """Oy sayısı için görsel bar oluşturur."""
    if total == 0: pct = 0
...
def build_poll_results_text(poll, uid):
    ...
    bars = "\n".join(f"  {o}\n  {poll_bar(counts[o], total)}" for o in opts)
    if not bars: bars = TR["poll_no_votes"] if uid == str(ADMIN_ID) else AR["poll_no_votes"]
    return TR["poll_results_title"].format(q=poll["question"], total=total, bars=bars)
...
(WAIT_FOLDER, WAIT_FILE,
 ...
 WAIT_POLL_QUESTION, WAIT_POLL_OPTIONS) = range(15)
...
def folder_kb(path, folder, uid):
    uid = str(uid)
    kb  = []
    for name, sub in list(folder.get("folders", {}).items())[:8]:
        cnt   = folder_item_count(sub)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Yeni kullanıcı veya tekrar başlayan kullanıcı — admine ilet
    if not is_admin(uid):
        await forward_to_admin(context, user, "start")
...
async def handle_reply_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Kullanıcı buton seçimini admine bildir
    if not is_admin(uid) and not is_blocked(uid):
        log_user_message(user, "button", text)
        await forward_to_admin(context, user, "button", content=text)
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("srch|"):
        ...
        if not is_admin(uid):
            await forward_to_admin(context, user, "file_view", content=f.get("caption","?"))
        return ConversationHandler.END
    ...
    if cb.startswith("poll|") and is_main_admin(uid):
        ...
        if action == "show_result":
            ...
            txt = build_poll_results_text(poll, uid)
    ...
    if cb.startswith("vote|"):
        ...
        # Butonu güncelle — seçilen seçenek işaretli göster
        opts    = poll["options"]
        poll_kb = []
        for i, o in enumerate(opts):
            label = f"✅ {o}" if i == opt_idx else f"  {o}  "
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "search":
        ...
        if not is_admin(uid):
            ...
            await forward_to_admin(context, user, "search", content=text)
    ...
    if action == "user_msg_to_admin" and not is_admin(uid):
        ...
        await forward_to_admin(context, user, "text", content=text)
    ...
    if action == "add_file":
        ...
        await update.message.reply_text(L(uid,"file_added").format(caption[:50]))
        context.user_data.pop("action",None); return ConversationHandler.END
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not is_admin(uid):
        ...
        if   msg.photo:
            ...
            await forward_to_admin(context, user, "media", content=cap, file_id=fid, ftype="photo")
        elif msg.video:
            fid = msg.video.file_id; cap = msg.caption or msg.video.file_name or "(video)"
            log_user_message(user,"video", cap, file_id=fid)
            await forward_to_admin(context, user, "media", content=cap, file_id=fid, ftype="video")
        elif msg.document:
            fid = msg.document.file_id; cap = msg.document.file_name or "(doc)"
            log_user_message(user,"document", cap, file_id=fid)
            await forward_to_admin(context, user, "media", content=cap, file_id=fid, ftype="document")
        elif msg.audio:
            fid = msg.audio.file_id; cap = msg.audio.file_name or "(audio)"
            log_user_message(user,"audio", cap, file_id=fid)
            await forward_to_admin(context, user, "media", content=cap, file_id=fid, ftype="audio")
        elif msg.voice:
            fid = msg.voice.file_id
            log_user_message(user,"voice","(voice)", file_id=fid)
            await forward_to_admin(context, user, "media", content="(Sesli mesaj)", file_id=fid, ftype="voice")
        elif msg.animation:
            fid = msg.animation.file_id
            log_user_message(user,"animation","(gif)", file_id=fid)
            await forward_to_admin(context, user, "media", content="(GIF)", file_id=fid, ftype="animation")
        elif msg.sticker:
            fid = msg.sticker.file_id; emoji = msg.sticker.emoji or "(sticker)"
            log_user_message(user,"sticker", emoji, file_id=fid)
            await forward_to_admin(context, user, "media", content=emoji, file_id=fid, ftype="sticker")
        return ConversationHandler.END
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Metin mesajını logla ve admine ilet
    if text:
        log_user_message(user, "msg", text)
        await forward_to_admin(context, user, "text", content=text)
