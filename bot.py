...
ADMIN_ID       = 7731559022
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyDr2jt-pa2t3HpkFBwxM3U_yNSVMLfHE3A")
...
def reply_kb(uid):
    ...
    if is_main_admin(uid):
        ...
        # Kullanıcılar: AI + arama + AI toggle + yardım
        ai_lbl = ai_btn_label(uid, AR)
        return ReplyKeyboardMarkup([
            [KeyboardButton(AR["btn_chat"]),     KeyboardButton(AR["btn_search"])],
            [KeyboardButton(ai_lbl),             KeyboardButton(AR["btn_help"])],
...
def main():
    ...
    print(f"🤖 Gemini AI    : {'✅ Aktif' if GEMINI_API_KEY else '⚠️  KEY EKSİK'}")
    if "/data" in BASE_DIR:
