...
TR = {
    ...
    "reminder_when":      "⏰ Ne zaman hatırlatayım?\n\nÖrnekler:\n• 2 saat\n• 30 dakika\n• 1 gün\n• 1.5 saat",
...
AR = {
    ...
    "reminder_when":      "⏰ متى تريد التذكير؟\n\nأمثلة:\n• 2 ساعة\n• 30 دقيقة\n• 1 يوم\n• نصف ساعة",
...
def add_user_note(uid: str, note_type: str, content: str, file_id: str = None):
...
async def do_search(message, uid: str, query: str):
    ...
    for r in results[:10]:
        ...
        if r["is_folder"]:
            # Klasöre git butonu
            folder_path = r["path"] + [r["name"]] if r["name"] not in r["path"] else r["path"]
            fp_enc = "~~".join(folder_path)
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if cb.startswith("srch|"):
        # goto_folder için de buradan geçelim
        pass
    ...
    if cb.startswith("notes|") and not is_admin(uid):
        ...
        if action_n == "add":
            ...
            prompts = {
                ...
                "photo": "🖼 أرسل صورة لحفظها كملاحظة:" if not is_main_admin(uid) else "🖼 Resim gönder:",
                "voice": "🎙 أرسل رسالة صوتية:" if not is_main_admin(uid) else "🎙 Ses mesajı gönder:",
            }
            await query.edit_message_text(
            ...
            return WAIT_FILE if note_type in ("photo","voice") else WAIT_FOLDER
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "add_note" and not is_admin(uid):
        ...
        add_user_note(uid, note_type, text)
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == "add_note" and not is_admin(uid):
        ...
        add_user_note(uid, note_type, cap, file_id=fid)
