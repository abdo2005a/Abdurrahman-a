...
def reply_kb(uid):
    if is_main_admin(uid):
        rows = [
            [KeyboardButton(MGMT_BTN_TR)],
            [KeyboardButton(CONTENT_BTN_TR)],
            [KeyboardButton(SETTINGS_BTN_TR)],
        ]
    elif is_admin(uid):
        rows = [
            [KeyboardButton(MGMT_BTN_AR)],
            [KeyboardButton(CONTENT_BTN_AR)],
        ]
    else:
...
def build_inline_keyboard(path, folder, uid, admin=False):
    ...
    if admin:
        # 1) Yönetim | İçerik Yönetimi (iki büyük buton)
        if is_main_admin(uid):
            kb.append([
                InlineKeyboardButton("👥 Yönetim",          callback_data="group_management"),
                InlineKeyboardButton("📁 İçerik Yönetimi",  callback_data="group_content"),
            ])
        else:
            kb.append([
                InlineKeyboardButton("👥 Yönetim",          callback_data="group_management"),
        ...
        # 2) Bot Ayarları (sadece ana admin)
        if is_main_admin(uid):
            kb.append([InlineKeyboardButton(t(uid, "btn_bot_settings"), callback_data="bot_settings")])

        # 3) Bakım
        s = load_settings()
        kb.append([
            InlineKeyboardButton(
                t(uid, "btn_maint_off") if s["maintenance"] else t(uid, "btn_maint_on"),
                callback_data="maintenance_off" if s["maintenance"] else "maintenance_on",
            )
        ])
